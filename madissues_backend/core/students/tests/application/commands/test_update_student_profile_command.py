...
from madissues_backend.core.students.application.commands.update_student_profile_command import \
    UpdateStudentProfileCommand, ChangeStudentProfileRequest
...
class TestUpdateStudentProfileCommand(unittest.TestCase):
    def setUp(self):
        self.db = EntityTable()
    ...
    def test_update_student_profile_successful(self):
        ...
        # Initialize command
        command = UpdateStudentProfileCommand(authentication_service, self.student_repository, self.event_bus)

        # Create request
        request = ChangeStudentProfileRequest(
            degree=str(GenericUUID.next_id()),
            joined_courses=[str(GenericUUID.next_id()) for _ in range(2)]
        )
        ...
        self.assertTrue(response.is_success(), "Profile should be updated")
        self.assertEqual(response.success.student_id, str(self.student.id), "Response should contain student_id")
        self.assertEqual(self.student_repository.get_by_id(self.student.id).profile.degree, GenericUUID(request.degree),
                         "Degree should be updated in db")
        self.assertEqual(self.student_repository.get_by_id(self.student.id).profile.joined_courses,
                         [GenericUUID(joined_course) for joined_course in request.joined_courses],
                         "Joined courses should be updated in db")
        self.assertEqual(len(self.event_bus.events), 1, "Should trigger an event")

    def test_update_student_profile_unauthenticated(self):
        # Reset the event bus (important)
        self.event_bus.events.clear()
        ...
        self.assertTrue(response.is_error(), "Should not be able to update profile without authentication")
