...
class TestMockTaskManagerRepository(unittest.TestCase):
    ...
    def test_add_task_manager(self):
        task_manager_config = TaskManagerConfig(service="trello",
                                                api_key="test_key",
                                                api_token="test_token")  # replace with actual values
        task_manager = TaskManager(id=GenericUUID.next_id(), organization_id=GenericUUID.next_id(),
                                   config=task_manager_config, task_manager_project_id=str(GenericUUID.next_id()))
    ...
    def test_check_can_integrate_organization_no_organization(self):
        ...
        self.assertFalse(
            self.repo.check_can_integrate_organization(str(GenericUUID.next_id()), str(GenericUUID.next_id())))
